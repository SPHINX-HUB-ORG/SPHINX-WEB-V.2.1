...
const upsertLinkAtSelection = (editor, {
  ...
  if (wrap && plateCore.isCollapsed(editor.selection)) {
    const linkLeaf = plateCore.getLeafNode(editor, editor.selection);
    const [, inlinePath] = linkLeaf;
    plateCore.select(editor, inlinePath);
  }
...
const getAndUpsertLink = async (editor, getLinkUrl) => {
  const type = plateCore.getPluginType(editor, ELEMENT_LINK);
  let prevUrl = '';
  const linkNode = plateCore.getAboveNode(editor, {
    match: {
      type
    }
  });
...
const upsertLinkIfValid = (editor, {
  ...
  const textFromBlockStart = plateCore.getEditorString(editor, rangeFromBlockStart);
...
const withLink = (editor, {
  ...
  editor.insertText = text => {
    if (text === ' ' && plateCore.isCollapsed(editor.selection)) {
      ...
      if (beforeWordRange) {
        const beforeWordText = plateCore.getEditorString(editor, beforeWordRange);
