...
const addColumn = (editor, {
  ...
  if (plateCore.someNode(editor, {
    ...
    const currentCellItem = plateCore.getAboveNode(editor, {
      match: {
...
const getTableCellEntry = (editor, {
  ...
  if (at && plateCore.someNode(editor, {
    ...
    const tableCell = plateCore.getAboveNode(editor, {
      at,
      match: {
        type: [plateCore.getPluginType(editor, ELEMENT_TD), plateCore.getPluginType(editor, ELEMENT_TH)]
      }
    }) || plateCore.getParentNode(editor, paragraphPath);
    if (!tableCell) return;
    ...
    const tableRow = plateCore.getParentNode(editor, tableCellPath);
    if (!tableRow) return;
    const [tableRowNode, tableRowPath] = tableRow;
    if (tableRowNode.type !== plateCore.getPluginType(editor, ELEMENT_TR)) return;
    const tableElement = plateCore.getParentNode(editor, tableRowPath);
    ...
    if (cellEntries.length > 1) {
      plateCore.withoutNormalizing(editor, () => {
        cellEntries.forEach(([, cellPath]) => {
          for (const [, childPath] of plateCore.getNodeChildren(editor, cellPath, {
            reverse: true
          })) {
            plateCore.removeNodes(editor, {
              at: childPath
      ...
      plateCore.select(editor, cellEntries[0][1]);
      return;
    }
          ...
          plateCore.withoutNormalizing(editor, () => {
            ...
            insertedRows.forEach(row => {
              ...
              insertedCells.forEach(cell => {
                ...
                plateCore.insertNodes(editor, cloneDeep_1(cell), {
                  at: cellPath
                });
