...
const getBlockAbove = (editor, options = {}) => slate$1.getAboveNode(editor, { ...options,
  block: true
});
...
const getPointBeforeLocation = (editor, at, options) => {
  ...
  matchStrings.some(matchString => {
    ...
    while (true) {
      ...
      const beforePoint = slate$1.getPointBefore(editor, beforeAt, options); // not found
...
const getPointNextToVoid = (editor, {
  ...
  if (startVoid) {
    ...
    if (blockAbove) {
      ...
      if (after) {
        nextPoint = slate$1.getPointAfter(editor, at);
      } else {
        nextPoint = slate$1.getPointBefore(editor, at);
      }
...
const getPreviousBlockById = (editor, id, query) => {
  ...
  const _nodes = slate$1.getNodeEntries(editor, {
    ...
    match: n => {
      ...
      if (!slate$1.isBlock(editor, n) || !n.id) return false; // find the block then take the previous one.
...
const isPointAtWordEnd = (editor, {
  ...
  // Point after at
  const after = slate$1.getPointAfter(editor, at); // From at to after

  const afterRange = slate$1.getRange(editor, at, after);
  const afterText = slate$1.getEditorString(editor, afterRange); // Match regex on after text
...
const isWordAfterTrigger = (editor, {
  ...
  const before = wordBefore && slate$1.getPointBefore(editor, wordBefore); // Range from before to start

  const beforeRange = before && slate$1.getRange(editor, before, at); // Before text

  const beforeText = slate$1.getEditorString(editor, beforeRange); // Starts with char and ends with word characters
...
const queryEditor = (editor, {
  ...
  if (allows.length && !slate$1.someNode(editor, {
    at,
    match: {
      type: allows
    }
  ...
  if (excludes.length && slate$1.someNode(editor, {
    at,
    match: {
      type: excludes
    }
...
const insertElements = (editor, nodes, options) => slate$1.insertNodes(editor, nodes, options);
...
const removeNodeChildren = (editor, path, options) => {
  slate$1.withoutNormalizing(editor, () => {
    for (const [, childPath] of slate$1.getNodeChildren(editor, path, {
      ...
      slate$1.removeNodes(editor, { ...options,
        at: childPath
      });
    }
  });
...
const replaceNodeChildren = (editor, {
  ...
  slate$1.withoutNormalizing(editor, () => {
    ...
    slate$1.insertNodes(editor, nodes, { ...insertOptions,
      at: at.concat([0])
    });
  });
};
...
const wrapNodeChildren = (editor, element, options) => {
  ...
  slate$1.withoutNormalizing(editor, () => {
    ...
    slate$1.wrapNodes(editor, element, { ...options,
      at: firstChildPath
    });
