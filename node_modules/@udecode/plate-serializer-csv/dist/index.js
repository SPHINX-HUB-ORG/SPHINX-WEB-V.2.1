...
const deserializeCsv = (editor, {
  ...
  } = plateCore.getPlugin(editor, KEY_DESERIALIZE_CSV); // Verify it's a csv string
  ...
  if (testCsv.errors.length === 0) {
    ...
    const paragraph = plateCore.getPluginType(editor, plateCore.ELEMENT_DEFAULT);
    const table = plateCore.getPluginType(editor, plateTable.ELEMENT_TABLE);
    const th = plateCore.getPluginType(editor, plateTable.ELEMENT_TH);
    const tr = plateCore.getPluginType(editor, plateTable.ELEMENT_TR);
    const td = plateCore.getPluginType(editor, plateTable.ELEMENT_TD);
    const ast = {
      type: table,
      children: []
    };
...
const createDeserializeCsvPlugin = plateCore.createPluginFactory({
