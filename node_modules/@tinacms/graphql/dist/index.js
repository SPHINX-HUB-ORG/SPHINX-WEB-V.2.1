...
var require_xhtml = __commonJS({
  "../../../node_modules/.pnpm/acorn-jsx@5.3.2_acorn@8.7.1/node_modules/acorn-jsx/xhtml.js"(exports, module2) {
    module2.exports = {
      quot: '"',
      amp: "&",
      apos: "'",
...
var require_acorn = __commonJS({
  "../../../node_modules/.pnpm/acorn@8.7.1/node_modules/acorn/dist/acorn.js"(exports, module2) {
...
var require_acorn_jsx = __commonJS({
  "../../../node_modules/.pnpm/acorn-jsx@5.3.2_acorn@8.7.1/node_modules/acorn-jsx/index.js"(exports, module2) {
    "use strict";
  ...
  constructor(config) {
    ...
    this._buildObjectOrUnionData = async (collectableTemplate, extraFields = [], extraInterfaces = [], collection) => {
      if (collectableTemplate.type === "union") {
        ...
        const typeMap = {};
        const types2 = await sequential(collectableTemplate.templates, async (template) => {
        ...
        return astBuilder.UnionTypeDefinition({ name: name2, types: types2 });
      }
  ...
  constructor(config) {
    ...
    this.getTemplateForData = async ({
      ...
      switch (templateInfo.type) {
        ...
        case "union":
          assertShape(data3, (yup3) => yup3.object({ _template: yup3.string().required() }));
          const template = templateInfo.templates.find((template2) => template2.namespace[template2.namespace.length - 1] === data3._template);
          if (!template) {
            throw new TinaGraphQLError(`Expected to find template named '${data3._template}' for collection '${lastItem(collection.namespace)}'`, {
...
var VFile = class {
  ...
  set path(path5) {
    if (isUrl(path5)) {
      path5 = (0, import_url.fileURLToPath)(path5);
    }
...
function assertPath(path5, name2) {
  if (!path5) {
...
function compiler(options = {}) {
  const config = configure({
    ...
    enter: {
      ...
      htmlFlow: opener(html2, buffer3),
      htmlFlowData: onenterdata,
      htmlText: opener(html2, buffer3),
...
function containerPhrasing(parent, context, safeOptions) {
  ...
  while (++index2 < children.length) {
    ...
    if (index2 + 1 < children.length) {
      ...
      after = handle2 ? handle2(children[index2 + 1], parent, context, __spreadValues({
        before: "",
        after: ""
...
// ../../../node_modules/.pnpm/mdast-util-mdx-jsx@1.2.0/node_modules/mdast-util-mdx-jsx/index.js
...
// ../../../node_modules/.pnpm/mdast-util-mdx@1.1.0/node_modules/mdast-util-mdx/index.js
var mdxFromMarkdown = [
...
// ../../../node_modules/.pnpm/remark-mdx@2.0.0-rc.2/node_modules/remark-mdx/index.js
...
var parseMDX = (value, field, graphQLconfig, schema) => {
  const tree = unified().use(remark_parse_default).use(remarkMdx).parse(value);
    ...
    case "rich-text":
      if (attribute.value) {
        if (field.name === "children") {
          ...
          try {
            const mdx = parseMDX(attribute.value.value, field, graphQLconfig, schema);
        ...
        console.log("nothing", field.name, remarkMdx);
      }
      break;
  }
};
...
function remarkToSlate(node, graphQLconfig, schema) {
  const types2 = __spreadProps(__spreadValues({}, defaultNodeTypes), {
    heading: __spreadValues({}, defaultNodeTypes.heading)
  });
  switch (node.type) {
    case "heading":
      return {
        type: types2.heading[node.depth],
        children: node.children.map((node2) => remarkToSlate(node2, graphQLconfig, schema))
      };
    case "list":
      return {
        type: node.ordered ? types2.ol_list : types2.ul_list,
        children: node.children.map((node2) => remarkToSlate(node2, graphQLconfig, schema))
      };
    case "listItem":
      const realChildren = [];
      node.children.forEach((child) => {
        if (child.type === "list") {
          realChildren.push({
            type: child.ordered ? types2.ol_list : types2.ul_list,
    ...
    case "blockquote":
      return {
        type: types2.block_quote,
        children: node.children.map((node2) => remarkToSlate(node2, graphQLconfig, schema))
      };
    case "code":
      return {
        type: types2.code_block,
        lang: node.lang,
    ...
    case "emphasis":
      return __spreadValues(__spreadValues({
        [types2.emphasis_mark]: true
      }, forceLeafNode(node.children)), persistLeafFormats(node.children));
    case "strong":
      return __spreadValues(__spreadValues({
        [types2.strong_mark]: true
      }, forceLeafNode(node.children)), persistLeafFormats(node.children));
    case "delete":
      return __spreadValues(__spreadValues({
        [types2.delete_mark]: true
      }, forceLeafNode(node.children)), persistLeafFormats(node.children));
    case "inlineCode":
      return {
        [types2.inline_code_mark]: true,
        text: node.value
      };
    case "thematicBreak":
      return {
        type: types2.thematic_break,
    ...
    case "break":
      return { type: types2.break, children: [{ type: "text", text: "" }] };
    default:
...
function blockquote(node, _4, context, safeOptions) {
...
function code(node, _4, context, safeOptions) {
  ...
  if (node.lang) {
    ...
    value += tracker.move(safe(context, node.lang, __spreadValues({
      before: value,
      after: " ",
      encode: ["`"]
    }, tracker.current())));
  ...
  if (node.lang && node.meta) {
    ...
    value += tracker.move(" ");
    value += tracker.move(safe(context, node.meta, __spreadValues({
      before: value,
      after: "\n",
      encode: ["`"]
    }, tracker.current())));
...
function definition2(node, _4, context, safeOptions) {
  const quote = checkQuote(context);
  ...
  value += tracker.move(safe(context, association(node), __spreadValues({
    before: value,
    after: "]"
  }, tracker.current())));
  value += tracker.move("]: ");
  ...
  if (!node.url || /[\0- \u007F]/.test(node.url)) {
    ...
    value += tracker.move("<");
    value += tracker.move(safe(context, node.url, __spreadValues({ before: value, after: ">" }, tracker.current())));
    value += tracker.move(">");
  } else {
    subexit = context.enter("destinationRaw");
    value += tracker.move(safe(context, node.url, __spreadValues({
      before: value,
      after: node.title ? " " : "\n"
    }, tracker.current())));
  }
  ...
  if (node.title) {
    ...
    value += tracker.move(safe(context, node.title, __spreadValues({
      before: value,
      after: quote
    }, tracker.current())));
    value += tracker.move(quote);
...
function emphasis(node, _4, context, safeOptions) {
  const marker = checkEmphasis(context);
  const exit2 = context.enter("emphasis");
  const tracker = track(safeOptions);
  let value = tracker.move(marker);
  value += tracker.move(containerPhrasing(node, context, __spreadValues({
    before: value,
    after: marker
  }, tracker.current())));
...
function heading(node, _4, context, safeOptions) {
  const rank = Math.max(Math.min(6, node.depth || 1), 1);
  const tracker = track(safeOptions);
  if (formatHeadingAsSetext(node, context)) {
    const exit3 = context.enter("headingSetext");
    const subexit2 = context.enter("phrasing");
    const value2 = containerPhrasing(node, context, __spreadProps(__spreadValues({}, tracker.current()), {
      before: "\n",
      after: "\n"
    }));
    subexit2();
  ...
  let value = containerPhrasing(node, context, __spreadValues({
    before: "# ",
    after: "\n"
  }, tracker.current()));
  if (/^[\t ]/.test(value)) {
...
function image(node, _4, context, safeOptions) {
  const quote = checkQuote(context);
  const suffix = quote === '"' ? "Quote" : "Apostrophe";
  ...
  const tracker = track(safeOptions);
  let value = tracker.move("![");
  value += tracker.move(safe(context, node.alt, __spreadValues({ before: value, after: "]" }, tracker.current())));
  value += tracker.move("](");
  subexit();
...
function imageReference(node, _4, context, safeOptions) {
  const type = node.referenceType;
  const exit2 = context.enter("imageReference");
  let subexit = context.enter("label");
  const tracker = track(safeOptions);
  let value = tracker.move("![");
  const alt = safe(context, node.alt, __spreadValues({
    before: value,
    after: "]"
  }, tracker.current()));
  ...
  const reference = safe(context, association(node), __spreadValues({
    before: value,
    after: "]"
  }, tracker.current()));
  subexit();
...
link.peek = linkPeek;
function link(node, _4, context, safeOptions) {
  const quote = checkQuote(context);
  const suffix = quote === '"' ? "Quote" : "Apostrophe";
  const tracker = track(safeOptions);
  let exit2;
  ...
  if (formatLinkAsAutolink(node, context)) {
    ...
    value2 += tracker.move(containerPhrasing(node, context, __spreadValues({
      before: value2,
      after: ">"
    }, tracker.current())));
    value2 += tracker.move(">");
...
function linkReference(node, _4, context, safeOptions) {
  const type = node.referenceType;
...
function paragraph(node, _4, context, safeOptions) {
...
function root(node, _4, context, safeOptions) {
...
function strong(node, _4, context, safeOptions) {
  ...
  constructor(init) {
    ...
    this.getDocument = async (fullPath) => {
      ...
      try {
        ...
        try {
          await sequential(template.fields, async (field) => {
            return this.resolveFieldData(field, rawData, data3);
          });
        } catch (e) {
        ...
        const title = data3[titleFieldName || " "] || null;
  ...
  constructor(config) {
    ...
    this.get = async (filepath) => {
      if (SYSTEM_FILES.includes(filepath)) {
        ...
        const templateName = hasOwnProperty3(contentObject, "_template") && typeof contentObject._template === "string" ? contentObject._template : void 0;
    ...
    this.addPendingDocument = async (filepath, data3) => {
      const { stringifiedFile, payload, keepTemplateKey } = await this.stringifyFile(filepath, data3);
    ...
    this.put = async (filepath, data3, collection) => {
      try {
        if (SYSTEM_FILES.includes(filepath)) {
    ...
    this.stringifyFile = async (filepath, data3) => {
      if (SYSTEM_FILES.includes(filepath)) {
        throw new Error(`Unexpected put for config file ${filepath}`);
      } else {
        const tinaSchema = await this.getSchema();
        ...
        if (templateInfo.type === "union") {
          if (hasOwnProperty3(data3, "_template")) {
            template = templateInfo.templates.find((t) => lastItem(t.namespace) === data3._template);
          } else {
        ...
        if (["md", "mdx"].includes(collection.format) && field) {
          ...
          payload["$_body"] = data3[field.name];
        } else {
          payload = data3;
        }
        const extension2 = import_path4.default.extname(filepath);
    ...
    this.flush = async (filepath) => {
      const data3 = await this.get(filepath);
      const { stringifiedFile } = await this.stringifyFile(filepath, data3);
...
var _indexContent = async (database, documentPaths, collection) => {
  ...
  await sequential(documentPaths, async (filepath) => {
    try {
      ...
      const data3 = parseFile(dataString, import_path4.default.extname(filepath), (yup3) => yup3.object({}));
      if (database.store.supportsSeeding()) {
        await database.store.seed(normalizePath(filepath), data3, seedOptions);
      }
    } catch (error) {
